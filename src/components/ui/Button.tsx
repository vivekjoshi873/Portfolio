        ...
        try {
            ...
            const content = (
                <>
                    {loading && <LoadingSpinner />}
                    {iconName && iconPosition === 'left' && renderIcon()}
                    {React.Children.only(child as React.ReactElement<{ children: React.ReactNode }>).props?.children}
                    {iconName && iconPosition === 'right' && renderIcon()}
                </>
            );
            ...
            const clonedChild = React.cloneElement(child as React.ReactElement<any>, {
                className: cn(
                    buttonVariants({ variant, size, className }),
                    fullWidth && "w-full",
                    React.Children.only(child as React.ReactElement<{ className: string }>).props?.className
                ),
                disabled: disabled || loading || React.Children.only(child as React.ReactElement<{ disabled: boolean }>).props?.disabled,
                children: content,
            });
